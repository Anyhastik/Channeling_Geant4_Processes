...
public:
	...
	G4ChanRad(const G4String& processName = "XTRenergyLoss", G4ProcessType type = fElectromagnetic);
	virtual ~G4ChanRad();
	virtual void GetThettaCrit(G4double& thetacrit);
	...
	static constexpr ld1 num = n_steps * nbeam;
	static constexpr G4int poi = G4int (num); 
	void ActivateProcesses(const G4Step& aStep);
	void DeactivateProcesses(const G4Step& aStep);

	void GetTrajectories(vector<ld1> &x_path, vector<ld1>&time_moving,
...
protected:
	...
	static constexpr G4double fMinProtonTkin = 400. * CLHEP::GeV;
	...
	vector<ld1> TT;
	vector<ld1> x_path;
	vector<ld1>  time_moving;
	vector<ld1>  x_velocity;
	vector<ld1>  z_path;
	vector<ld1>  z_velocity;
	//copies of arrays
	vector<ld1> x_path_copy;
	vector<ld1> time_moving_copy;
	vector<ld1> x_velocity_copy;
	vector<ld1> z_path_copy;
	vector<ld1> z_velocity_copy;
	int tables_count;
