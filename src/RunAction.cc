...
RunAction::RunAction()
  : G4UserRunAction(),
    fRunMessenger(0), fRndmFreq(1), fFileName("output.root")
{
  ...
  analysisManager->CreateNtuple("ntuple", "Step data");
  ...
  analysisManager->CreateNtupleSColumn("secondaryParticleType");
  analysisManager->CreateNtupleDColumn("secondaryEnergyMeV");
  ...
  analysisManager->CreateNtupleDColumn("ProcessType");
...
{  
  ...
  //    G4cout << "Could not open output file." << G4endl;
  //}
  //if ( analysisManager->IsActive() ) {
  //  analysisManager->OpenFile();
  //  G4cout << "HHHHHHHHH" << G4endl;
  //}
  G4cout << "Begin of Run Action" << G4endl;
  auto analysisManager = G4AnalysisManager::Instance();
  if (!analysisManager->OpenFile()) {
      G4cerr << "Could not open output file output.root: " << fFileName << G4endl;
  }
  else {
      G4cout << "Output file opened successfully: " << fFileName << G4endl;
  }   
...
void RunAction::EndOfRunAction(const G4Run* run)
{
  ...
  /*if ( analysisManager->IsActive() ) {
    ...
    G4cout << "wrires" << G4endl;
    analysisManager->CloseFile();
  }  */
  G4cout << "End of Run Action" << G4endl;
  //auto analysisManager = G4AnalysisManager::Instance();
  if (analysisManager->Write()) {
      G4cout << "Data written to file successfully output.root" << G4endl;
  }
  else {
      G4cerr << "Failed to write data to file output.root" << G4endl;
  }
  if (analysisManager->CloseFile()) {
      G4cout << "Output file closed successfully output.root" << G4endl;
  }
  else {
      G4cerr << "Failed to close output file output.root" << G4endl;
  }
